...
class CausalExplainer:
    ...
    def generate_causal_scores(
        ...
        lat_inputs = torch.split(lat, batch_size) if lat is not None else [None] * len(inputs)
        lon_inputs = torch.split(lon, batch_size) if lon is not None else [None] * len(inputs)
        aux_inputs = torch.split(aux_data, batch_size) if aux_data is not None else [None] * len(inputs)
        static_inputs = torch.split(static_vars, batch_size) if static_vars is not None else [None] * len(inputs)
        relAs, relKs = [], []

        for data, lat_b, lon_b, aux_b, static_b in zip(inputs, lat_inputs, lon_inputs, aux_inputs, static_inputs):
            relA, relK = self._generate_RRP(
                data,
                target_series_idx,
                lat=lat_b,
                lon=lon_b,
                aux_data=aux_b,
                static_vars=static_b,
            )
            relAs.append(relA)
            relKs.append(relK)
    ...
    def _generate_RRP(self, input_data, target_series_idx, lat=None, lon=None, aux_data=None, static_vars=None):
        """
        内部方法：使用 RRP 生成因果分数
        ...
        # output = self.model(input_data, lat, lon, aux_data, static_vars)
        output = self.model(input_data,None,None,aux_data,static_vars)
    ...
    def interpret_causal_graph(
        ...
        """
        relA, relK = self.generate_causal_scores(
            input_data,
            target_series_idx,
            batch_size,
            lat=lat,
