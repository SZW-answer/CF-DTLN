...
def draw_centered_title(draw, box, text, font):
...
def stitch_columns(items, output_path, dpi=OUTPUT_DPI):
    ...
    title_h = max(170, round(target_height * TITLE_RATIO))
    gutter = max(70, round(target_height * GUTTER_RATIO))
    width = sum(image.width for _, image in images) + gutter * (len(images) - 1)
    height = target_height + title_h
    canvas = Image.new("RGB", (width, height), "white")
    draw = ImageDraw.Draw(canvas)
    title_font = make_font(max(72, round(title_h * 0.46)), bold=True)

    x = 0
    for title, image in images:
        draw_centered_title(draw, (x, 0, x + image.width, title_h), title, title_font)
